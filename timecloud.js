...
(function($) {
	...
	ui: function(e) {
            ...
            var timegraph=this.buildSparklineRange(
                {
                    ...
                    range: true,
                    change: function(e,ui) {return thisObj.overviewZoomed(e,ui)},
                },
                this.overviewMoved);
        ...
        overviewZoomed: function (e, ui) {
            if(this.overviewElem.window.slider('value', 0)==ui.value) {
                this.options.start = this.overviewElem.window.slider('value', 0);
            }
            if(this.options.winSize == this.frames.length && ui.range < this.frames.length) {
                this.timecloudElem.slideDown();
            }
            if(this.options.winSize < this.frames.length && ui.range >= this.frames.length) {
                this.timecloudElem.slideUp();
            }
            this.options.winSize = Math.round(ui.range);
            this.drawTimecloud();
        },
        ...
        redrawOverviewRange: function() {
            ...
            var right = left + this.options.winSize;
            return this.redrawRange(this.overviewElem.window, left, right);
        },

        // internal function to update the handles of windows.
        redrawRange: function(w,left,right) {
